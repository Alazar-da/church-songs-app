...
import { FaArrowLeft, FaSave, FaUserPlus, FaUserShield, FaCrown, FaCheck, FaSpinner, FaFolder, FaEnvelope, FaLock, FaUser } from 'react-icons/fa'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: existingUser } = await supabase
        .from('profiles')
        .select('id')
        .eq('id', formData.email) // This won't work, but we'll check differently
      ...
      if (roleError || !roleData) {
        throw new Error(`Role "${formData.role}" not found. Please make sure roles exist in the database.`)
      }
      ...
      // Assign categories for category admin
      if (formData.role === 'category_admin' && formData.category_ids.length > 0) {
        ...
        if (assignError) {
          console.error('Category assignment error:', assignError)
          // Don't throw here, just log - profile was created successfully
          toast('Admin created but category assignment failed', { icon: '⚠️' })
        }
      }
      
      toast.success('Admin created successfully! They can now log in.')
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <div>
            ...
            <p className="text-sm sm:text-base text-primary-600 mt-1 sm:mt-2">
              Add a new administrator to manage church songs
            </p>
          </div>
        </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-11 pr-24 py-3 bg-primary-50 border border-primary-200 rounded-xl text-church-dark placeholder-primary-400 focus:outline-none focus:ring-2 focus:ring-church-gold focus:border-transparent transition-all duration-300"
                      placeholder="••••••••"
                      required
                    />
                    <button
                      ...
                      className="absolute right-4 top-1/2 transform -translate-y-1/2 text-sm text-primary-500 hover:text-church-gold transition-colors"
                    >
